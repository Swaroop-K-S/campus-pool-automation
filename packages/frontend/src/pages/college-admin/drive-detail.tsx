...
const CountdownTimer = ({ closeDate }: { closeDate: string }) => {
  ...
  return (
    <div
      className={`border rounded-xl p-3 flex w-full items-center gap-3 ${
        urgency === 'urgent'
...
const DEFAULT_FIELDS = [
  { id: 'field_name', type: 'text', label: 'Full Name', required: true, locked: true, order: 0 },
  {
    ...
    validation: {
      ...
      customErrorMessage: 'Must be a valid alphanumeric USN/Roll No',
    },
  },
  {
    id: 'field_email',
    ...
    validation: {
      pattern: '^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$',
      customErrorMessage: 'Valid email required',
    },
  },
  {
    id: 'field_phone',
    ...
    locked: true,
    order: 3,
    validation: { pattern: '^\\d{10}$', customErrorMessage: 'Must be exactly 10 digits' },
  },
  {
    id: 'field_gender',
    type: 'dropdown',
  ...
  },
  {
    ...
    locked: true,
    order: 6,
    validation: { min: 0, max: 10, customErrorMessage: 'CGPA must be between 0 and 10' },
  },
  ...
  const renderHighlighted = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('{{') && part.endsWith('}}')) {
        return (
          <span key={i} className="bg-indigo-100 text-transparent rounded-sm inline-block">
            {part}
          </span>
        );
      }
...
export default function DriveDetailPage() {
  ...
  const [emailSubject, setEmailSubject] = useState(
    'Invitation: {{companyName}} Campus Drive — {{eventDate}}',
  );
    ...
    const fetchConflict = async () => {
      try {
        const res: any = await api.get(
          `/drives/schedule/check-conflict?date=${eventDate}&excludeDriveId=${driveId}`,
        );
  ...
  const [liveEvents, setLiveEvents] = useState<
    { id: number; text: string; type: 'info' | 'warning' | 'critical' }[]
  >([]);
  ...
  const fetchMIAStudents = async () => {
    ...
    try {
      const [miaRes, latecomerRes] = await Promise.all([
        ...
        api
          .get(`/drives/${driveId}/applications?status=all&limit=200`)
          .then((r: any) =>
  ...
  const [lastWalkIn, setLastWalkIn] = useState<{ driveStudentId: string; name: string } | null>(
    null,
  );
  ...
  const saveApplicationData = async () => {
    ...
    try {
      ...
      setApplications((prev) =>
        prev.map((a) => (a._id === selectedApp._id ? { ...a, data: editedAppData } : a)),
      );
    } catch (err) {
  ...
  const fetchApplications = async (page = 1, append = false) => {
    ...
    try {
      const res = await api.get(
        `/drives/${driveId}/applications?status=${appStatusFilter}&page=${page}&limit=50`,
      );
      if ((res as any).success) {
  ...
  const runEligibilitySweep = async () => {
    ...
    try {
      ...
      if ((res as any).success) {
        toast.success(
          `Eligibility sweep complete! ${(res as any).data.rejectedCount} applications rejected.`,
        );
  ...
  const handlePurgeStaleQueue = async () => {
    if (
      !window.confirm(
        'Are you absolutely sure you want to PURGE ALL stale students? This applies to students checked-in over 45 minutes ago who were never assigned to or processed in a room. This moves them into the Rejected pile to clear up active queues.',
      )
    )
      return;
  ...
  const getStudentName = (s: any) => {
    ...
    const nk =
      Object.keys(d).find((k) => k.toLowerCase() === 'fullname') ||
      Object.keys(d).find((k) => k.toLowerCase() === 'full_name') ||
      Object.keys(d).find((k) => k.toLowerCase() === 'name') ||
      Object.keys(d).find(
        (k) => k.toLowerCase().includes('name') && !k.toLowerCase().includes('email'),
      );
    return (nk ? d[nk] : null) || d.name || d.Name || d.full_name || 'Unknown';
  };
  const getStudentEmail = (s: any) => {
    const d = s.data || {};
    const f = getFieldFromFormFields('email', d);
    if (f) return f;
    return (
      d.email ||
      d.Email ||
      d.email_id ||
      Object.values(d).find((v) => typeof v === 'string' && String(v).includes('@')) ||
      (s as any).candidateEmail ||
      ''
    );
  };
  const getStudentPhone = (s: any) => {
    const d = s.data || {};
    const f = getFieldFromFormFields('phone', d) || getFieldFromFormFields('mobile', d);
    if (f) return f;
    return d.phone || d.Phone || d.mobile || d.contact || '';
  };
  const getStudentUSN = (s: any) => {
    const d = s.data || {};
    const f = getFieldFromFormFields('usn', d) || getFieldFromFormFields('roll', d);
    if (f) return f;
    return d.usn || d.USN || d.roll_no || '—';
  };
  const getStudentBranch = (s: any) => {
    const d = s.data || {};
    const f = getFieldFromFormFields('branch', d) || getFieldFromFormFields('department', d);
    if (f) return f;
    return d.branch || d.Branch || '—';
  };
  const getStudentCGPA = (s: any) => {
    const d = s.data || {};
    const f = getFieldFromFormFields('cgpa', d) || getFieldFromFormFields('gpa', d);
    if (f) return f;
    return String(d.cgpa || d.CGPA || '—');
  };
  ...
  const resolvePreview = (template: string, student: any): string => {
    ...
    const vars: Record<string, string> = {
      ...
      usn: getStudentUSN(student),
      branch: getStudentBranch(student),
      cgpa: getStudentCGPA(student),
      email: String(getStudentEmail(student)),
      phone: String(getStudentPhone(student)),
      driveId: student.driveStudentId || '—',
      ...
      eventDate: drive?.eventDate
        ...
        : 'TBA',
      venueName: (drive as any)?.venueDetails?.hallName || 'TBA',
  ...
  const saveRoom = async (roundName: string) => {
    try {
      const panelists = newRoom.panelists
        ? newRoom.panelists
            .split(',')
            .map((p) => ({ name: p.trim(), expertise: [...newExpertiseTags] }))
        : [];
      const payload = {
        name: newRoom.name,
        floor: newRoom.floor,
  ...
  const handleArchiveDrive = async () => {
    if (
      !confirm(
        'Are you sure you want to archive this drive and download the compliance report? Applications cannot be processed after archival.',
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/drives/${driveId}/archive`,
        {
          credentials: 'include',
        },
      );
  ...
  const updateField = updateActiveField; // alias used by Properties Drawer
  ...
  const scheduleForm = async () => {
    try {
      const res = await api.patch(`/drives/${driveId}/form/schedule`, {
        formOpenDate,
        formCloseDate,
      });
      if ((res as any).success) {
      ...
      toast.error('Failed to schedule form');
    }
  };

  const extendForm = async () => {
    try {
      const res = await api.patch(`/drives/${driveId}/form/extend`, {
        newCloseDate,
        reason: extendReason,
      });
      if ((res as any).success) {
  ...
  if (loading)
    return <div className="p-8 text-center text-slate-500 font-bold">Loading drive details...</div>;
    ...
    <div className="flex flex-col h-full absolute inset-0 bg-slate-50">
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Link
                to="/admin/dashboard"
                className="inline-flex items-center justify-center w-8 h-8 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span
                    className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-widest flex items-center gap-1 border ${
                      drive.status === 'active'
                  ...
                  >
                    {(drive.status === 'active' || drive.status === 'event_day') && (
                      <span className="relative flex h-1.5 w-1.5 shrink-0">
                        <span
                          className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${drive.status === 'active' ? 'bg-emerald-500' : 'bg-indigo-500'}`}
                        ></span>
                        <span
                          className={`relative inline-flex rounded-full h-1.5 w-1.5 ${drive.status === 'active' ? 'bg-emerald-500' : 'bg-indigo-500'}`}
                        ></span>
                      </span>
                    )}
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all hover:shadow-md group relative ${
                  drive.status === 'event_day'
                    ? 'text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border-indigo-200/60 hover:shadow-indigo-100'
              ...
              <button
                onClick={() => {
                  const url = `${window.location.origin}/event/${driveId}/my-status`;
                  navigator.clipboard
                    .writeText(url)
                    .then(() => toast.success('Status page URL copied!'));
                }}
      ...
      <div className="bg-white/80 backdrop-blur-md border-b border-slate-200/60 px-8 flex gap-8 shrink-0 relative z-10 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] overflow-x-auto hide-scrollbar">
        <div className="max-w-7xl mx-auto flex gap-6 w-full h-14">
          {[
            { id: 'Config', label: 'Engine Setup' },
            { id: 'Pipeline', label: 'The Pipeline' },
            { id: 'CommandCenter', label: 'Command Center' },
            { id: 'LaunchControl', label: '🚀 Launch Control' },
          ].map((hub) => (
            <button
              key={hub.id}
              onClick={() => {
                setActiveHub(hub.id);
                if (hub.id === 'Config') setActiveTab('Form Builder');
                else if (hub.id === 'Pipeline') setActiveTab('Overview');
              }}
          ...
          <div className="max-w-5xl mx-auto w-full">
            ...
            <div className="w-full bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-between relative overflow-hidden mb-6">
              <div className="flex items-center gap-2 mb-6 absolute top-0 right-0 bg-indigo-50 px-4 py-2 rounded-bl-2xl border-b border-l border-indigo-100">
                ...
                />
                <span className="text-xs font-black uppercase text-indigo-800 tracking-wider flex items-center gap-1">
                  {drive?.status?.replace('_', ' ')}
                </span>
              </div>
              ...
              <div className="flex items-center border border-slate-100 rounded-2xl justify-between w-full mt-4 h-32 px-6 bg-slate-50 relative">
                ...
                {[
                  ...
                  {
                    label: 'Interviewed',
                    count: appStats?.interview_passed || 0,
                    color: 'text-pink-600',
                    bg: 'bg-pink-100',
                  ...
                  <div
                    key={idx}
                    className="flex flex-col items-center flex-1 relative z-10 hover:-translate-y-1 transition-transform cursor-default"
                  >
                    <div
                      className={
                        'w-16 h-16 rounded-2xl flex items-center justify-center font-black text-2xl shadow-sm border ring-4 ring-slate-50 z-10 ' +
                        step.bg +
                        ' ' +
                        step.color +
                        ' ' +
                    ...
                    <span className="text-xs font-bold text-slate-600 mt-3 uppercase tracking-wider bg-white px-2 py-0.5 rounded-md border border-slate-200 shadow-sm">
                      {step.label}
                    </span>
                  </div>
                ))}
            ...
            <div className="flex justify-start">
              <div className="flex bg-slate-200/60 p-1.5 rounded-xl shadow-inner border border-slate-200/80">
                <button
                  ...
                  className={
                    'px-8 py-2 text-sm font-black rounded-lg transition-all tracking-wide ' +
                    (activeTab === 'Overview'
                ...
                <button
                  ...
                  className={
                    'px-8 py-2 text-sm font-black rounded-lg transition-all tracking-wide ' +
                    (activeTab === 'Shortlist'
                      ? 'bg-white text-emerald-700 shadow-sm ring-1 ring-emerald-900/10'
                      : 'text-slate-500 hover:text-slate-800')
                  }
                >
                  Shortlist Grid
      ...
      <div className="flex-1 overflow-hidden relative">
          ...
          <div className="p-8 overflow-y-auto custom-scrollbar h-full flex flex-col gap-8 bg-[#F8FAFC]">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 relative overflow-hidden shrink-0">
              ...
              <div className="flex items-center justify-between mb-8 relative z-10">
                <div>
                  <h3 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2">
                    <Cpu className="text-indigo-500" /> Pipeline Operations Center
                  </h3>
                  <p className="text-sm font-semibold text-slate-500 mt-1">
                    Accelerate drive execution with one-click global actions
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 relative z-10">
                    ...
                    try {
                      ...
                      if ((res as any).success) {
                        ...
                        if (matchedCandidates.length > 0) {
                          toast.success(
                            `Found ${matchedCandidates.length} eligible candidates! (${alreadyApplied} applied). Invitations dispatched!`,
                            { id: toastId, duration: 5000 },
                          );
                        } else {
                          toast.success(`No new candidates matched your strict criteria.`, {
                            id: toastId,
                          });
                        }
                      }
                ...
                >
                  <div className="w-12 h-12 rounded-2xl bg-emerald-100 text-emerald-600 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:bg-emerald-500 group-hover:text-white transition-all shadow-sm">
                    <UserCheck size={22} />
                  </div>
                  ...
                  <span className="text-xs font-semibold text-slate-500">
                    Run AI engine to dispatch invitations
                  </span>
                  <div className="absolute top-4 right-4 text-emerald-300 group-hover:text-emerald-500 transition-colors">
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      `${window.location.origin}/apply/${drive.formToken}`,
                    );
                ...
                >
                  <div className="w-12 h-12 rounded-2xl bg-slate-200 text-slate-600 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-all shadow-sm">
                    <Copy size={22} />
                  </div>
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-slate-500">
                    Edit form structure and dependencies
                  </span>
                  <div className="absolute top-4 right-4 text-slate-300 group-hover:text-indigo-400 transition-colors">
                    <ChevronRight size={20} />
                  </div>
                ...
                >
                  <div className="flex items-center justify-between w-full mb-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm transition-all group-hover:scale-110 ${drive.walkInEnabled ? 'bg-amber-100 text-amber-600' : 'bg-slate-200 text-slate-600 group-hover:bg-amber-100 group-hover:text-amber-600'}`}
                    >
                      <UserPlus size={22} />
                    </div>
                    <div
                      className={`w-10 h-5 rounded-full p-0.5 transition-colors ${drive.walkInEnabled ? 'bg-amber-500' : 'bg-slate-300 group-hover:bg-slate-400'}`}
                    >
                      <div
                        className={`bg-white w-4 h-4 rounded-full shadow-sm transition-transform ${drive.walkInEnabled ? 'translate-x-5' : 'translate-x-0'}`}
                      ></div>
                    </div>
                  </div>
                  <span className="font-bold flex items-center gap-2 text-base mb-1 text-slate-800">
                    Walk-In Fast Track
                    {drive.walkInEnabled ? (
                      <span className="text-[10px] bg-amber-200 text-amber-800 px-1.5 py-0.5 rounded font-black uppercase tracking-wider">
                        Active
                      </span>
                    ) : null}
                  </span>
                  <span
                    className={`text-xs font-semibold ${drive.walkInEnabled ? 'text-amber-700/80' : 'text-slate-500'}`}
                  >
                    Enable instant panel generation
                  </span>
                </button>
                ...
                <DriveTemplateManager
                  ...
                  trigger={
                    <button className="flex flex-col items-start w-full p-5 bg-gradient-to-br from-violet-50/50 to-white hover:from-white hover:to-violet-50 border border-violet-100 hover:border-violet-300 hover:shadow-md rounded-2xl transition-all group text-left relative h-full">
                      <div className="w-12 h-12 rounded-2xl bg-violet-100 text-violet-600 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:bg-violet-500 group-hover:text-white transition-all shadow-sm">
                        <BookOpen size={22} />
                      </div>
                      <span className="font-bold text-slate-800 text-base mb-1">
                        Save as Template
                      ...
                      <span className="text-xs font-semibold text-slate-500">
                        Persist execution rules and forms
                      </span>
                ...
                {drive.status !== 'completed' && (
                  ...
                  >
                    <div className="w-12 h-12 rounded-2xl bg-red-100 text-red-600 flex items-center justify-center mb-4 group-hover:scale-110 transition-all shadow-sm group-hover:bg-red-500 group-hover:text-white">
                      <Lock size={22} />
                    </div>
                    <span className="font-bold text-red-900 text-base mb-1">
                      Halt Registrations
                    </span>
                    <span className="text-xs font-semibold text-red-700/70">
                      Instantly block new applicants
                    </span>
                  </button>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 shrink-0">
              ...
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 flex flex-col relative overflow-hidden h-full">
                <h3 className="font-black text-slate-800 text-lg flex items-center gap-2 mb-6">
                  <BarChart2 className="text-blue-500" size={20} /> Event Parameters
                </h3>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  ...
                  <div className="bg-emerald-50 border border-emerald-100 p-5 rounded-2xl border-b-4 border-b-emerald-200">
                    <p className="text-emerald-600 text-xs font-black uppercase tracking-wider mb-2">
                      Pending
                    </p>
                    <p className="text-4xl font-black text-emerald-700">
                      {Math.max(0, appStats.total - appStats.shortlisted)}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4 flex-1 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                  <div className="flex justify-between items-center border-b border-white pb-4">
                    ...
                    <span className="font-black text-slate-800 text-sm bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">
                      {drive.jobRole}
                    </span>
                  </div>
                  <div className="flex justify-between items-center border-b border-white pb-4">
                    <span className="text-slate-500 font-bold text-sm">Target Offer</span>
                    <span className="font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-500 text-xl">
                      {drive.ctc}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 font-bold text-sm">Engine Status</span>
                    <span className="font-black text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 shadow-sm text-sm">
                      Active
                    </span>
                  </div>
                </div>
              ...
              <div className="xl:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-slate-200 flex flex-col h-full">
                <div className="flex items-center justify-between mb-8">
                  <h3 className="font-black text-slate-800 text-lg flex items-center gap-2">
                    <CheckSquare className="text-indigo-500" size={20} /> Execution Strategy
                  </h3>
                  <button
                    onClick={() => {
                      setActiveHub('Config');
                      setActiveTab('Event Day');
                    }}
                    className="text-indigo-600 text-sm font-bold hover:text-indigo-700 hover:bg-indigo-100 flex items-center gap-1 bg-indigo-50 px-4 py-2 rounded-xl border border-indigo-100 transition-colors shadow-sm"
                  >
                    Launch Event Console <ChevronRight size={16} />
                  </button>
                </div>
                ...
                <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                  {drive.rounds?.map((r: any, idx: number) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className={`flex items-center justify-between p-5 rounded-2xl border transition-all ${isActive ? 'bg-indigo-50 border-indigo-200 shadow-sm filter drop-shadow-sm' : isDone ? 'bg-slate-50 border-slate-200 opacity-80' : 'bg-white border-slate-200 hover:border-slate-300'}`}
                      >
                        <div className="flex items-center gap-6">
                          <div
                            className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl ${isDone ? 'bg-emerald-100 text-emerald-600 shadow-inner' : isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-slate-100 text-slate-400'}`}
                          >
                            {isDone ? <Check size={28} /> : r.order || idx + 1}
                          </div>
                          <div>
                            <h4
                              className={`font-black text-base md:text-lg tracking-tight ${isActive ? 'text-indigo-900' : 'text-slate-800'}`}
                            >
                              {r.label ||
                                r.type
                                  ?.replace(/_/g, ' ')
                            ...
                            <div className="text-xs font-bold mt-2 tracking-wide flex flex-wrap items-center gap-2">
                              {isDone ? (
                                <span className="text-emerald-700 bg-emerald-100 border border-emerald-200 px-2 py-0.5 rounded-md shadow-sm">
                                  Module Completed
                                </span>
                              ) : isActive ? (
                                <span className="text-indigo-700 bg-indigo-100 border border-indigo-200 px-2 py-0.5 rounded-md shadow-sm flex items-center gap-1.5">
                                  <span className="w-2 h-2 rounded-full bg-indigo-600 animate-pulse" />{' '}
                                  Executing Live
                                </span>
                              ) : (
                                <span className="text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-md shadow-sm">
                  ...
                  {(!drive.rounds || drive.rounds.length === 0) && (
                    <div className="flex flex-col items-center justify-center py-16 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                      <div className="w-20 h-20 bg-white shadow-sm rounded-full flex items-center justify-center text-slate-400 mb-5">
                        <List size={32} />
                      </div>
                      <p className="text-slate-600 font-bold text-lg">
                        No execution modules found.
                      </p>
                      <p className="text-slate-400 text-sm mt-2 max-w-sm">
                        Configure your drive rules to set up automatic test and interview rounds.
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden shrink-0">
              ...
              >
                ...
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center bg-slate-50 border border-slate-100 transition-transform ${isPolicyExpanded ? 'rotate-90 bg-violet-50 text-violet-600 border-violet-100' : 'text-slate-400 group-hover:bg-white line-shadow'}`}
                >
                  <ChevronRight size={20} />
                </div>
              </button>
              ...
              {isPolicyExpanded && (
                <div className="border-t border-slate-100 px-8 pb-8 pt-6 space-y-6 bg-slate-50/30">
                  ...
                  <div className="bg-white p-6 border border-slate-200 rounded-2xl shadow-sm">
                    <label className="text-xs font-black text-slate-600 uppercase tracking-wider block mb-4 flex items-center gap-2">
                      <BarChart2 size={16} /> Minimum CGPA Strict Filter
                    </label>
                    <div className="flex items-center gap-6">
                      <input
                        type="range"
                  ...
                  <div className="bg-white p-6 border border-slate-200 rounded-2xl shadow-sm">
                    <label className="text-xs font-black text-slate-600 uppercase tracking-wider block mb-4 flex items-center gap-2">
                      <List size={16} /> Allowed Branches Restriction
                    </label>
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-center gap-4 shadow-sm">
                    ...
                    <p className="text-sm text-amber-800 font-semibold leading-relaxed">
                      Saving will fundamentally update drive eligibility rules globally. Click{' '}
                      <strong className="text-amber-900 bg-amber-200/50 px-2.5 py-0.5 rounded ml-1 mr-1">
                        "Run Sweep"
                      </strong>{' '}
                      immediately afterwards to auto-reject non-eligible applicants from the entire
                      database.
                    </p>
                  </div>
                ...
                <div className="relative z-10 flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 mt-2 max-w-xl text-sm font-medium leading-relaxed">
                      Download a secure, master Excel file containing all applications, shortlist
                      decisions, and communication audit logs for external sharing and
                      record-keeping.
                    </p>
                  </div>
                </div>
        ...
        {activeHub === 'Config' && activeTab === 'Form Builder' && (
          <div
            className="flex h-full w-full bg-slate-50 overflow-hidden"
          ...
          >
            ...
            <div className="w-64 bg-white border-r border-slate-200 p-4 overflow-y-auto shrink-0 z-10">
              ...
              <div className="mb-6 space-y-2">
                {DEFAULT_FIELDS.map((f) => (
                  <div
                    key={f.id}
                    className="flex items-center gap-3 p-2.5 bg-slate-50 rounded-lg border border-slate-100 text-slate-400 select-none"
                  >
            ...
            <div className="flex-1 flex flex-col p-6 overflow-y-auto relative border-r border-slate-200 bg-slate-50/50 custom-scrollbar pb-32">
                ...
                <div className="flex items-center gap-3">
                  {formToken && (
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(
                          `${window.location.origin}/apply/${formToken}`,
                        );
                        toast.success('Public form link copied!');
                      }}
                  ...
                  <button
                    onClick={() => {
                      ...
                      fetchDrivesForImport();
                    }}
                    className="px-3 py-1.5 bg-indigo-50 border border-indigo-100 text-indigo-700 hover:bg-indigo-100 rounded-lg text-xs font-bold transition-colors flex items-center gap-1"
                  >
              ...
              {/* Form Validity Period moved to Event Day tab */}
              {(formStatus === 'open' || formStatus === 'extended') && (
                <div className="mb-4 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between gap-4 animate-in fade-in">
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-sm font-bold text-emerald-800">
                      Form is {formStatus === 'extended' ? 'Extended' : 'Open'} — Accepting
                      Applications
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 border border-amber-400 text-amber-700 hover:bg-amber-50 rounded-lg text-xs font-bold"
                    >
                      Extend
                    </button>
                    <button
                      onClick={() => setShowCloseConfirm(true)}
                      className="px-3 py-1.5 border border-red-400 text-red-600 hover:bg-red-50 rounded-lg text-xs font-bold"
                    >
                      Close Now
                    </button>
                  </div>
              ...
              {formStatus === 'closed' && (
                <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 rounded-xl flex items-center justify-between gap-4 animate-in fade-in">
                  <span className="text-sm font-bold text-red-700">
                    ✕ Form is Closed — go to Event Day tab to reopen
                  </span>
                </div>
              )}
              ...
              {(formToken || drive.formToken) && (
                <div className="absolute top-6 left-1/2 -translate-x-1/2 w-full max-w-2xl z-20">
                  <div className="bg-green-50 border border-green-300 rounded-xl p-4 shadow-sm flex items-center justify-between">
                    <div>
                      <p className="text-xs font-bold text-green-800 uppercase tracking-widest mb-1 flex items-center gap-1">
                        🔗 Public Form Link
                      </p>
                      <a
                        href={`${window.location.origin}/apply/${formToken || drive.formToken}`}
                        target="_blank"
                        rel="noreferrer"
                        className="text-green-700 font-medium text-sm hover:underline hover:text-green-800"
                      >
            ...
            {/* PROPERTIES DRAWER — slides over preview when a field is selected */}
            {activeField && (
              <div
              ...
              >
                <div
                  className="w-full max-w-sm bg-white shadow-2xl overflow-y-auto flex flex-col border-l border-slate-200 animate-in slide-in-from-right duration-250"
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50 shrink-0">
                    <div>
                      ...
                      <h3 className="font-black text-slate-800 text-sm mt-0.5">
                  ...
                  <div className="flex-1 p-5 space-y-5 overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between py-2">
                      <div>
                        ...
                        <p className="text-xs text-slate-400">Students must fill this field</p>
                      </div>
                      <button
                        disabled={activeField.locked}
                        onClick={() => updateField({ required: !activeField.required })}
                        className={`w-12 h-6 rounded-full transition-colors relative ${activeField.required ? 'bg-indigo-600' : 'bg-slate-200'} ${activeField.locked ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                        <div
                          className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all ${activeField.required ? 'left-7' : 'left-1'}`}
                        />
                      </button>
                    </div>
                    ...
                    {['dropdown', 'radio', 'checkbox'].includes(activeField.type) && (
                      <div>
                        ...
                        <textarea
                          rows={5}
                          className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 outline-none resize-none font-mono"
                      ...
                      <div className="space-y-3">
                        {activeField.type === 'number' && (
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              ...
                              <input
                                type="number"
                                className="w-full border border-slate-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-indigo-400"
                        ...
                        {['text', 'email', 'phone'].includes(activeField.type) && (
                          <div>
                            ...
                            <input
                              className="w-full border border-slate-200 rounded-lg px-3 py-1.5 text-sm font-mono outline-none focus:ring-2 focus:ring-indigo-400"
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="e.g. Must be a valid 10-digit number"
                          />
                        </div>
                      </div>
                    </div>
            ...
            <div className="flex-1 bg-[#F8FAFC] overflow-y-auto p-4 lg:p-8 flex items-start justify-center relative shadow-[inset_4px_0_12px_rgba(0,0,0,0.02)] custom-scrollbar pb-32 border-r border-slate-200 hidden md:flex">
              <div className="w-full max-w-sm lg:max-w-md bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden sticky top-0 transition-all">
                <div className="h-28 bg-gradient-to-r from-indigo-500 to-violet-600 relative p-6">
                  <div className="w-14 h-14 bg-white rounded-2xl shadow-md absolute -bottom-7 flex items-center justify-center text-3xl border-4 border-white">
                    🎓
                  </div>
                </div>
                <div className="pt-10 p-6 space-y-5">
                  <h2 className="text-xl font-black text-slate-800 leading-tight">
                    {drive?.companyName || 'CampusPool'}
                    <br />
                    <span className="text-indigo-600">Application</span>
                  </h2>

                  {fields.length === 0 ? (
                    <div className="text-slate-400 text-sm py-4 border-t border-slate-100">
                        ...
                        <div key={f.id} className="opacity-90">
                          ...
                          {f.type === 'text' && (
                            <input
                              readOnly
                              type="text"
                              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none cursor-default"
                          ...
                          {f.type === 'textarea' && (
                            <textarea
                              readOnly
                              rows={2}
                              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none cursor-default resize-none"
                              placeholder={f.placeholder || 'Type here...'}
                            />
                          )}
                          {f.type === 'select' && (
                            <select
                              disabled
                              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none cursor-default appearance-none"
                            >
                              <option>Select option</option>
                            </select>
                          )}
                          {f.type === 'file' && (
                            <div className="border border-dashed border-slate-300 rounded-lg p-3 bg-slate-50/50 text-center text-xs font-semibold text-slate-400 cursor-default">
                              Upload {f.label}
                            </div>
                          )}
                  ...
                  <button
                    disabled
                    className="w-full py-3 mt-2 bg-slate-100 text-slate-400 rounded-xl font-bold text-xs cursor-not-allowed"
                  >
                    Submit Application
                  </button>
                </div>
            ...
            <div className="absolute bottom-6 right-6 z-20">
              <button
                onClick={saveForm}
                disabled={savingForm}
                className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl shadow-[0_8px_20px_-8px_rgba(79,70,229,0.6)] flex items-center gap-2 hover:-translate-y-0.5 transition-all outline-none"
              >
        ...
        {activeHub === 'Pipeline' &&
          ...
          (() => {
            ...
            const tableFields = (() => {
              ...
              const rawKeys = Object.keys(firstApp.data).filter((k) => {
            ...
            const safeStr = (val: any): string => {
              ...
              if (Array.isArray(val))
                return val.filter((v) => typeof v === 'string' || typeof v === 'number').join(', ');
              return '—';
            };
            ...
            const getFieldValue = (app: any, field: any) => {
              ...
              if (label.includes('usn') || label.includes('roll') || label.includes('reg')) {
                const v =
                  appData.usn || appData.USN || appData.roll_no || appData.rollno || appData.reg_no;
                if (v) return safeStr(v);
              }
              ...
              <div className="p-8 h-full overflow-y-auto">
                ...
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="bg-slate-100 text-slate-600 text-sm px-2.5 py-1 rounded-full font-medium">
                      {appTotal}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 relative">
                    {appTotal > 0 &&
                      ...
                      (drive.eligibility?.minCGPA ||
                        (drive.eligibility?.branches && drive.eligibility.branches.length > 0)) && (
                        <button
                          onClick={() => setShowSweepConfirmModal(true)}
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white border border-transparent rounded-xl px-4 py-2 text-sm font-bold transition-all shadow-[0_4px_12px_-4px_rgba(79,70,229,0.5)]"
                    >
                      <Sparkles size={15} /> AI Match Candidates
                    </button>
                              ...
                              <div className="flex items-center justify-between p-4 border-b border-slate-100">
                                <div>
                                  ...
                                  <p className="text-xs text-slate-400 mt-0.5">
                                    {selectedColumns.length} of {allCols.length} selected
                                  </p>
                                </div>
                                <div className="flex gap-2">
                                  <button
                                  ...
                                  <div className="px-2 py-1.5 border-t border-slate-100 mt-1">
                                    ...
                                    {formColumns.map((col: any) => (
                                      ...
                                      >
                                        ...
                                        <span
                                          className={`text-xs px-1.5 py-0.5 rounded ${col.type === 'email' ? 'bg-blue-50 text-blue-600' : col.type === 'phone' ? 'bg-green-50 text-green-600' : col.type === 'number' ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 text-slate-400'}`}
                                        >
                                          {col.type}
                                        </span>
                                      </label>
                              ...
                              <div className="p-4 border-t border-slate-100">
                                <div className="mb-3">
                                  <label className="text-xs font-medium text-slate-600 block mb-1.5">
                                    Download Which Students?
                                  </label>
                                  <select
                                    value={downloadStatus}
                                    onChange={(e) => setDownloadStatus(e.target.value)}
                                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 bg-white focus:outline-none focus:border-indigo-400"
                                  >
                                ...
                                <button
                                  onClick={async () => {
                                    ...
                                    try {
                                      ...
                                      const response = await fetch(
                                        `${apiBase}/drives/${driveId}/export/applications/custom`,
                                        {
                                          method: 'POST',
                                          credentials: 'include',
                                ...
                                >
                                  {downloadLoading ? (
                                    ...
                                    <>
                                      <Download size={15} /> Download {selectedColumns.length}{' '}
                                      columns
                                    </>
                                  )}
                                </button>
                  ...
                  <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200">
                      ...
                      <p className="text-sm text-slate-600 mb-6 leading-relaxed">
                        This will automatically scan all <b>Applied</b> and <b>Shortlisted</b>{' '}
                        candidates. Anyone who does not meet the{' '}
                        <span className="font-semibold text-slate-800">
                          Min CGPA ({drive.eligibility?.minCGPA || 'N/A'})
                        </span>{' '}
                        or allowed branches will be permanently moved to{' '}
                        <span className="text-red-600 font-bold">Rejected</span> status.
                      </p>
                      <div className="flex gap-3">
                        <button
                          onClick={() => setShowSweepConfirmModal(false)}
                          className="flex-1 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-sm transition-colors"
                        >
                          Cancel
                        </button>
                        <button
                          onClick={runEligibilitySweep}
                          disabled={isSweepingEligibility}
                          className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2"
                        >
                          {isSweepingEligibility ? (
                            <>
                              <Loader2 size={16} className="animate-spin" /> Sweeping...
                            </>
                          ) : (
                            'Yes, Run Sweep'
                          )}
                ...
                <div className="flex gap-2 mb-4">
                  {['all', 'applied', 'shortlisted', 'attended', 'selected', 'exceptions'].map(
                    (status) => (
                      <button
                        key={status}
                  ...
                  {displayApps.length === 0 && (
                    <div className="absolute inset-0 z-20 bg-white/70 backdrop-blur-[2px] flex flex-col items-center justify-center p-8 text-center m-4 rounded-xl border-dashed border-2 border-slate-200">
                      ...
                      <p className="text-sm text-slate-500 font-medium max-w-sm">
                        When candidates apply, their detailed profiles and form responses will
                        populate this master list.
                      </p>
                    </div>
                  )}
                    ...
                    <table className="w-full relative">
                      <thead className="sticky top-0 z-10 bg-slate-50/90 backdrop-blur-md shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
                        <tr className="border-b border-slate-200">
                          <th className="text-left px-4 py-3 text-xs font-bold text-slate-600 uppercase tracking-widest w-8">
                            #
                          </th>
                          <th className="text-left px-4 py-3 text-xs font-bold text-slate-600 uppercase tracking-widest min-w-[12rem]">
                            Candidate
                          </th>
                        ...
                        {appLoading ? (
                          ...
                          <tr>
                            <td colSpan={tableFields.length + 5} className="px-4 py-16 text-center">
                              ...
                              <p className="text-slate-400 text-sm mt-1">
                                {applications.length === 0
                                  ? 'Share the form link to start collecting applications'
                          ...
                          displayApps.map((app: any, index: number) => {
                            ...
                            if (fullName === '—') {
                              ...
                              const nameKey =
                                nameKeys.find((k) => k.toLowerCase() === 'fullname') ||
                                nameKeys.find((k) => k.toLowerCase() === 'full_name') ||
                                nameKeys.find((k) => k.toLowerCase() === 'name') ||
                                nameKeys.find(
                                  (k) =>
                              ...
                              if (!resolved || typeof resolved === 'object') {
                                const nested = nameKeys.reduce((acc: string, k: string) => {
                                  ...
                                  if (typeof v === 'object' && v !== null && !Array.isArray(v)) {
                                    const innerKey = Object.keys(v).find((ik) =>
                                      ik.toLowerCase().includes('name'),
                                    );
                                    if (innerKey && typeof v[innerKey] === 'string')
                                      return v[innerKey];
                                  }
                                  return acc;
                                }, '');
                              ...
                              >
                                ...
                                <td className="px-4 py-3">
                                  <div className="flex items-center gap-3">
                                      ...
                                      <img
                                        src={
                                          app.photoUrl?.startsWith('http')
                                            ? app.photoUrl
                                            : app.photoUrl?.startsWith('/')
                                              ? `${apiBase}${app.photoUrl.replace('/api/v1', '')}`
                                              : `${apiBase}/drives/${driveId}/applications/${app._id}/photo`
                                        }
                                        alt={fullName}
                                    ...
                                    <div
                                      className={`w-9 h-9 rounded-full bg-indigo-100 text-indigo-700 text-sm font-bold flex items-center justify-center flex-shrink-0 ${app.hasPhoto ? 'hidden' : ''}`}
                                    >
                                      {initials}
                                    </div>
                                    <div>
                                      <div className="text-sm font-semibold text-slate-800 group-hover:text-indigo-700 transition-colors flex items-center gap-2">
                                        ...
                                        {app.data?.isExceptionFlagged && (
                                          <span
                                            className="text-[10px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full flex items-center shadow-sm"
                                            title="Flagged as Exception (Relaxed Criteria)"
                                          >
                                            ⚠️ Flagged
                                          </span>
                                        )}
                                        {app.studentProfileId?.strikes > 0 && (
                                          <span
                                            className="text-[10px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full flex items-center gap-1"
                                            title={`${app.studentProfileId.strikes} No-Show Strike(s) - Consider un-shortlisting if unreliable`}
                                          >
                                            <AlertCircle size={10} /> {app.studentProfileId.strikes}
                                          </span>
                                        )}
                                      </div>
                                      {app.driveStudentId ? (
                                        <div className="text-xs font-mono text-indigo-500 font-semibold">
                                          {app.driveStudentId}
                                        </div>
                                      ) : (
                                        <div className="text-xs text-slate-400">
                                          {app.referenceNumber ||
                                            app._id?.toString().slice(-8).toUpperCase()}
                                        </div>
                                      )}
                                    </div>
                                  </div>
                                ...
                                {tableFields.map((field: any) => {
                                  ...
                                  const isCgpa =
                                    field.type === 'number' &&
                                    field.label.toLowerCase().includes('cgpa');
                                  return (
                                    <td
                                      key={field.id}
                                      className="px-4 py-3 text-sm text-slate-700 max-w-[8rem] truncate"
                                    >
                                      {isCgpa ? (
                                        <span
                                          className={`font-semibold ${parseFloat(value) >= 8 ? 'text-green-600' : parseFloat(value) >= 6 ? 'text-amber-600' : 'text-red-500'}`}
                                        >
                                          {value}
                                        </span>
                                      ) : (
                ...
                {hasMoreApps && !appLoading && (
                  <div className="flex justify-center mt-6 mb-4">
                    <button
                      ...
                      className="px-6 py-2.5 bg-white border border-slate-200 shadow-sm text-indigo-600 font-bold rounded-full hover:bg-slate-50 transition-colors flex items-center gap-2"
                    >
                ...
                {showDetailDrawer &&
                  ...
                  (() => {
                    ...
                    if (drawerName === '—') {
                      ...
                      const dNameKey =
                        dNameKeys.find((k) => k.toLowerCase() === 'fullname') ||
                        dNameKeys.find((k) => k.toLowerCase() === 'full_name') ||
                        dNameKeys.find((k) => k.toLowerCase() === 'name') ||
                        dNameKeys.find(
                          (k) =>
                    ...
                    const drawerInitials = drawerName
                      .split(' ')
                      .map((n: string) => n[0])
                      .join('')
                      .toUpperCase()
                    ...
                    const dataFields = formFields.filter(
                      (f: any) => f.type !== 'file_pdf' && f.type !== 'file_image',
                    );
                        ...
                        <div className="fixed right-0 top-0 h-full w-full max-w-lg bg-white z-50 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                          ...
                          <div className="relative bg-gradient-to-br from-indigo-600 via-indigo-700 to-slate-900 p-6 pb-20">
                            <div className="flex items-start justify-between">
                              <div>
                                <span
                                  className={`inline-flex text-xs px-2.5 py-1 rounded-full font-bold uppercase tracking-wider ${
                                    app.status === 'selected'
                                ...
                                <p className="text-indigo-200/70 text-xs font-mono mt-2">
                                  {app.referenceNumber ||
                                    'REF-' + app._id?.toString().slice(-8).toUpperCase()}
                                </p>
                              </div>
                              <div className="flex items-center gap-1.5">
                                {!isEditingApp ? (
                                        ...
                                        <button
                                          onClick={() => {
                                            const apiBase =
                                              ...
                                              'http://localhost:5000/api/v1';
                                            window.open(
                                              `${apiBase}/drives/${driveId}/offer/${app._id}`,
                                              '_blank',
                                            );
                                          }}
                                        ...
                                        <button
                                          onClick={() => {
                                            const apiBase =
                                              ...
                                              'http://localhost:5000/api/v1';
                                            window.open(
                                              `${apiBase}/drives/${driveId}/noc/${app._id}`,
                                              '_blank',
                                            );
                                          }}
                                  ...
                                  <button
                                    ...
                                    className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/10 hover:bg-red-500/30 text-white/80 transition-colors print:hidden"
                                    title="Cancel Edit"
                                  >
                                    <X size={15} />
                                  </button>
                          ...
                          <div className="relative -mt-14 mx-6 mb-4">
                            <div className="bg-white rounded-2xl shadow-xl border border-slate-200/70 p-5 flex items-center gap-4">
                              <div className="relative flex-shrink-0">
                                {app.hasPhoto ? (
                                  <img
                                    src={`${apiBase}/drives/${driveId}/applications/${app._id}/photo`}
                                    alt={drawerName}
                                ...
                                <div
                                  className={`w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-100 to-indigo-200 text-indigo-700 text-2xl font-black flex items-center justify-center ${app.hasPhoto ? 'hidden' : ''}`}
                                >
                                  {drawerInitials}
                                </div>
                                <div
                              ...
                              <div className="flex-1 min-w-0">
                                <h2 className="text-xl font-black text-slate-800 flex items-center gap-2 flex-wrap">
                                  ...
                                  {app.studentProfileId?.strikes > 0 && (
                                    <span
                                      className="text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-full flex items-center gap-1"
                                      title="This student has missed previous drives they checked into"
                                    >
                                      <AlertCircle size={12} strokeWidth={3} />{' '}
                                      {app.studentProfileId.strikes} Strike
                                      {app.studentProfileId.strikes > 1 ? 's' : ''}
                                    </span>
                                  )}
                                </h2>
                                {app.driveStudentId && (
                                  <div className="flex items-center gap-1.5 mt-1">
                                    ...
                                    <button
                                      onClick={() => {
                                        navigator.clipboard.writeText(app.driveStudentId);
                              ...
                              <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-4 animate-in slide-in-from-top-2 duration-300">
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1">
                                    <h3 className="text-amber-800 font-bold text-sm tracking-tight mb-1">
                                      Guardian Exception Flagged
                                    </h3>
                                    <p className="text-amber-700/80 text-xs font-medium leading-relaxed mb-3">
                                      This candidate did not meet the requirement but applied under
                                      the <b>Relaxed</b> eligibility rule. Reason(s):
                                      {app.data.exceptionReasons?.map((r: string) => (
                                        <span
                                          key={r}
                                          className="ml-1 inline-flex bg-amber-100/50 px-1.5 rounded border border-amber-200"
                                        >
                                          {r}
                                        </span>
                                      ))}
                                    ...
                                    <div className="flex gap-2">
                                      <button
                                        onClick={async () => {
                                          try {
                                            await api.put(
                                              `/drives/${driveId}/applications/${app._id}/status`,
                                              { status: 'shortlisted' },
                                            );
                                            const updatedApp = {
                                              ...app,
                                              status: 'shortlisted',
                                              data: { ...app.data, isExceptionFlagged: false },
                                            };
                                            ...
                                            toast.success(
                                              'Exception approved! Candidate Shortlisted.',
                                            );
                                          } catch {
                                            toast.error('Failed to approve');
                                          }
                                ...
                                {app.hasPhoto ? (
                                  <div className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden">
                                    {app.photoUrl?.startsWith('http') ? (
                                      ...
                                      <SecureImage
                                        url={`/drives/${driveId}/applications/${app._id}/photo`}
                                        fallback={
                                          <div className="w-full h-48 bg-slate-100 flex items-center justify-center text-slate-400 font-bold">
                            ...
                            <div>
                              <div className="flex items-center justify-between mb-3">
                                ...
                                {isEditingApp && (
                                  <button
                                    ...
                                    className="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded shadow-sm text-xs font-bold disabled:opacity-50"
                                  >
                              ...
                              <div className="bg-slate-50 rounded-xl divide-y divide-slate-200 overflow-hidden border border-slate-200">
                                {dataFields.length > 0
                                  ? dataFields
                                      .map((field: any) => {
                                        const rawVal = isEditingApp
                                          ? editedAppData[field.id]
                                          : getFieldValue(app, field);
                                        ...
                                        return (
                                          <div
                                            key={field.id}
                                            className="flex items-start gap-4 px-4 py-3 hover:bg-slate-100/50 transition-colors"
                                          >
                                            <div className="text-xs text-slate-500 w-32 flex-shrink-0 pt-0.5 font-medium leading-relaxed">
                                              {field.label}
                                            </div>
                                            <div className="flex-1 text-sm text-slate-800 font-medium break-words">
                                              {isEditingApp ? (
                                                ...
                                                <span
                                                  className={`font-bold ${parseFloat(val) >= 8 ? 'text-green-600' : parseFloat(val) >= 6 ? 'text-amber-600' : 'text-red-500'}`}
                                                >
                                                  {val} / 10
                                                </span>
                                              ) : field.type === 'email' ? (
                                                ...
                                                <div className="flex items-center gap-2">
                                                  ...
                                                  <a
                                                    href={`https://wa.me/${String(val).replace(/\D/g, '')}?text=Hi%20${encodeURIComponent(drawerName)},%20regarding%20your%20campus%20placement%20application`}
                                                    target="_blank"
                                                    rel="noreferrer"
                                  ...
                                  : Object.entries(app.data || {})
                                      ...
                                      .map(([key, value]) => (
                                        <div key={key} className="flex items-start gap-4 px-4 py-3">
                                          <div className="text-xs text-slate-500 w-32 flex-shrink-0 pt-0.5 font-medium capitalize">
                                            {key.replace(/_/g, ' ')}
                                          </div>
                                          <div className="flex-1 text-sm text-slate-800 font-medium break-words">
                                            {isEditingApp ? (
                                              ...
                                              <div className="flex items-center gap-2">
                                                ...
                                                <a
                                                  href={`https://wa.me/${String(value).replace(/\D/g, '')}?text=Hi%20${encodeURIComponent(drawerName)},%20regarding%20your%20campus%20placement%20application`}
                                                  target="_blank"
                                                  rel="noreferrer"
                                                  title="Chat on WhatsApp"
                                                  className="text-emerald-500 hover:scale-110 transition-transform"
                            ...
                            <div>
                              ...
                              <div className="bg-slate-50 rounded-xl divide-y divide-slate-200 overflow-hidden border border-slate-200">
                                <div className="flex items-center gap-4 px-4 py-3">
                                  <div className="text-xs text-slate-500 w-32 font-medium">
                                    Reference
                                  </div>
                                ...
                                {app.attendedAt && (
                                  <div className="flex items-center gap-4 px-4 py-3">
                                    ...
                                    <div className="text-sm text-slate-800">
                                      {new Date(app.attendedAt).toLocaleString('en-IN')}
                                    </div>
                                  </div>
                                )}
                              </div>
                          ...
                          <div className="border-t border-slate-100 p-4 flex gap-3 bg-white">
                              ...
                              <a
                                href={
                                  app.resumeUrl && app.resumeUrl.startsWith('http')
                                    ? app.resumeUrl
                                    : `${apiBase}/drives/${driveId}/applications/${app._id}/resume`
                                }
                                target="_blank"
                                rel="noreferrer"
                            ...
                            {app.status === 'applied' && (
                              <button
                                onClick={() => {
                                  api
                                    .patch(`/drives/${driveId}/applications/${app._id}/status`, {
                                      status: 'shortlisted',
                                    })
                                    .then(() => {
            ...
            <div className="bg-white px-6 py-4 border-b border-slate-200 shadow-sm z-20 shrink-0">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors"
                >
              ...
              {uploadResult && (
                <div className="mt-6 bg-green-50 border border-green-200 rounded-xl p-5">
                  <p className="font-bold text-green-800 text-lg flex items-center gap-2 mb-2">
                    ✅ {uploadResult.matched} students matched and shortlisted
                  </p>
                  {uploadResult.notFound > 0 && (
                    <p className="font-bold text-amber-700 flex items-center gap-2">
                      ⚠️ {uploadResult.notFound} rows not found in database
                    </p>
                  )}
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              ...
              <div className="flex justify-between items-center px-6 py-3 border-b border-slate-200 bg-white">
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-indigo-100 text-indigo-700 text-sm px-2.5 py-0.5 rounded-full font-semibold">
                  ...
                  <div className="flex items-center gap-2 mr-2 border-r border-slate-200 pr-5 hidden md:flex">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (val === 'all_shortlisted') {
                          ...
                          const targetRoundIdx = drive.rounds.findIndex(
                            (r: any) => r.type === roundName,
                          );
                          targets = shortlistedStudents.filter((s: any) => {
                            if (s.status === 'rejected') return false; // Prevent rejected students from being targeted
                            if (s.status === 'selected' || s.currentRound === 'completed')
                              return true;
                            const studentRoundIdx = drive.rounds.findIndex(
                              (r: any) => r.type === s.currentRound,
                            );
                    ...
                    >
                      ...
                      {drive.rounds?.map((r: any) => (
                        <option key={r.type} value={`round_${r.type}`}>
                          Passed{' '}
                          {r.label ||
                            r.type
                              .replace(/_/g, ' ')
                ...
                <table className="w-full text-left relative border-collapse">
                  <thead className="sticky top-0 z-10 bg-slate-50/90 backdrop-blur-md shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
                    <tr className="border-b border-slate-200">
                      <th className="w-10 px-4 py-3">
                        <div
                          onClick={() => {
                            if (selectAllShortlist) {
                              setSelectedStudentIds(new Set());
                            } else {
                              setSelectedStudentIds(
                                new Set(shortlistedStudents.map((s: any) => s._id)),
                              );
                            }
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {shortlistedStudents.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="text-center py-24 bg-white">
                          ...
                          <p className="text-slate-500 text-sm font-medium">
                            Use the Dropzone above to import candidates via secure CSV/XLSX routing.
                          </p>
                        </td>
                      </tr>
                    ) : (
                      shortlistedStudents.map((student: any) => {
                        ...
                        return (
                          <tr
                            key={student._id}
                            className={`hover:bg-slate-50 transition-colors ${selectedStudentIds.has(student._id) ? 'bg-indigo-50/50' : ''}`}
                          >
                            <td className="px-4 py-3">
                              <div
                                onClick={() => toggleStudentSelect(student._id)}
                                className={`w-[18px] h-[18px] rounded border-2 cursor-pointer flex items-center justify-center transition-all ${selectedStudentIds.has(student._id) ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 hover:border-indigo-400'}`}
                              >
                                ...
                                <div>
                                  <div className="text-sm font-semibold text-slate-800 flex items-center gap-2">
                                    ...
                                    {student.studentProfileId?.strikes > 0 && (
                                      <span
                                        className="text-[10px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full flex items-center gap-1"
                                        title={`${student.studentProfileId.strikes} No-Show Strike(s)`}
                                      >
                                        <AlertCircle size={10} /> {student.studentProfileId.strikes}
                                      </span>
                                    )}
                                  </div>
                                  <div className="text-xs text-slate-400">
                            ...
                            <td className="px-4 py-3">
                              <span
                                className={`text-sm font-semibold ${parseFloat(cgpa) >= 8 ? 'text-green-600' : parseFloat(cgpa) >= 6 ? 'text-amber-600' : cgpa === '—' ? 'text-slate-400' : 'text-red-500'}`}
                              >
                                {cgpa}
                              </span>
                            </td>
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-1.5">
                                ...
                                {email && (
                                  <a
                                    href={`mailto:${email}?subject=${encodeURIComponent(`${drive?.companyName} Campus Drive Invitation`)}&body=${encodeURIComponent(buildQuickEmailBody(student))}`}
                                    title="Send Email"
                                ...
                                <button
                                  onClick={() => {
                                    setSelectedStudentIds(new Set([student._id]));
              ...
              (() => {
                ...
                return (
                  <div
                    ...
                    onClick={(e) => {
                      if (e.target === e.currentTarget) setShowTemplateBuilder(false);
                    }}
                  >
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500 mt-0.5">
                            ...
                            <strong className="text-indigo-600">
                              {isAll
                                ? `all ${targetCount} shortlisted students`
                          ...
                          <div className="flex gap-1 mb-5 p-1 bg-slate-100 rounded-xl w-fit">
                            {(['whatsapp', 'email', 'both'] as const).map((c) => (
                              <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${sendChannel === c ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                              >
                                {c === 'both'
                                  ? '📱📧 Both'
                                  : c === 'whatsapp'
                            ...
                            <>
                              {(sendChannel === 'whatsapp' || sendChannel === 'both') && (
                                <div className="mb-4">
                                  ...
                                  <div
                                    className="bg-[#DCF8C6] rounded-2xl rounded-tl-none p-3 text-sm text-slate-800 whitespace-pre-wrap leading-relaxed shadow-sm"
                                    style={{ fontSize: '13px' }}
                                  >
                              ...
                              {(sendChannel === 'email' || sendChannel === 'both') && (
                                <div>
                                  ...
                                  <div className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm">
                                    <div className="text-xs font-semibold text-slate-600 mb-1.5 pb-1.5 border-b border-slate-100">
                                      Subject: {resolvePreview(emailSubject, previewStudent)}
                                    </div>
                                    <div
                                      className="text-xs text-slate-700 whitespace-pre-wrap leading-relaxed"
                                      dangerouslySetInnerHTML={{
                                        __html: resolvePreview(emailTemplate, previewStudent),
                                      }}
                                    />
                                  </div>
                                </div>
                      ...
                      <div className="border-t border-slate-100 p-5 flex items-center justify-between bg-white rounded-b-2xl">
                        {sendProgress?.active ? (
                          ...
                          <div className="flex items-center gap-2 text-sm text-slate-500">
                            <Info size={14} />
                            Messages will be personalized for each student
                          </div>
                        )}
        ...
        {activeHub === 'Config' && activeTab === 'Event Day' && (
          <div className="p-8 h-full overflow-y-auto w-full max-w-5xl mx-auto space-y-6 pb-32">
            {/* ═══ FORM SCHEDULE CARD (moved from Form Builder canvas) ═══ */}
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-slate-800">Application Form Schedule</h3>
                <div
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    type="datetime-local"
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm mt-1 focus:ring-2 focus:ring-indigo-500 outline-none"
                    value={formOpenDate}
              ...
              <div className="flex gap-2 mb-3">
                ...
                {formStatus !== 'open' && (
                  <button
                    onClick={async () => {
                      const t = toast.loading('Opening form instantly...');
                      try {
                        await api.post(`/drives/${driveId}/form/schedule`, {
                          formOpenDate: new Date().toISOString(),
                          formCloseDate: formCloseDate || undefined,
                        });
                        toast.success('Form is now OPEN!', { id: t });
                        fetchDriveDetails();
                      } catch {
                        toast.error('Failed', { id: t });
                      }
                    }}
                    className="flex-1 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg font-bold text-sm transition-colors flex items-center justify-center gap-2"
                  >
                    <CheckCircle size={15} /> Start Accepting
                  </button>
                )}
              </div>
              {(formStatus === 'open' || formStatus === 'extended') && (
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <button
                      onClick={() => setShowExtendModal(true)}
                      className="flex-1 px-3 py-2 border border-amber-500 text-amber-600 hover:bg-amber-50 rounded-lg text-sm font-bold"
                    >
                      Extend Deadline
                    </button>
                    <button
                      onClick={() => setShowCloseConfirm(true)}
                      className="flex-1 px-3 py-2 border border-red-500 text-red-600 hover:bg-red-50 rounded-lg text-sm font-bold"
                    >
                      Close Form Now
                    </button>
                    <button
                      onClick={() => setShowHistoryModal(true)}
                      className="flex-1 px-3 py-2 border border-slate-300 text-slate-600 hover:bg-slate-50 rounded-lg text-sm font-bold"
                    >
                      History
                    </button>
                  </div>
              ...
              {formStatus === 'closed' && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex justify-between items-center">
                  <span className="text-red-700 font-bold text-sm">✕ Form is currently closed</span>
                  <button
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h3 className="text-lg font-semibold text-slate-800 border-b border-slate-100 pb-3 mb-5">
                ...
                <div className="mb-5 bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-700 mt-0.5">
                      There {conflictWarning.length === 1 ? 'is' : 'are'} already{' '}
                      {conflictWarning.length} drive(s) scheduled on this date:
                    </p>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-5">
                <div>
                  <h3 className="text-lg font-semibold text-slate-800">
                    Pre-Drive Preparation Materials
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">
                    Links visible to shortlisted students on their status page
                  </p>
                </div>
              ...
              <div className="flex gap-2 mb-4">
                ...
                <button
                  onClick={addResource}
                  className="flex-shrink-0 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-bold transition-colors"
                >
                  + Add
                </button>
              </div>
                        ...
                        <div className="pt-4 pb-6 pl-4">
                            ...
                            <div className="bg-slate-50 rounded-xl p-4 mb-4 border border-slate-200">
                              <div className="grid grid-cols-2 gap-3 mb-3">
                                ...
                                <input
                                  ...
                                  value={newRoom.panelists}
                                  onChange={(e) =>
                                    setNewRoom({ ...newRoom, panelists: e.target.value })
                                  }
                                  className="px-3 py-2 text-sm border border-slate-300 rounded-lg outline-none focus:border-indigo-500 font-medium"
                                />
                              </div>
                                ...
                                {newExpertiseTags.length > 0 && (
                                  <div className="flex flex-wrap gap-1.5 mt-2">
                                    {newExpertiseTags.map((tag, i) => (
                                      <span
                                        key={i}
                                        className="bg-indigo-100 text-indigo-700 rounded-full px-3 py-1 text-xs font-medium flex items-center gap-1"
                                      >
                                        {tag}
                                        <button
                                          onClick={() =>
                                            setNewExpertiseTags(
                                              newExpertiseTags.filter((_, idx) => idx !== i),
                                            )
                                          }
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            {roundRooms.map((rm) => (
                              <div
                                key={rm._id}
                                className="bg-white border border-slate-200 rounded-xl p-4 relative group hover:border-indigo-300 transition-colors"
                              >
                                {/* Top row */}
                                <div className="flex justify-between items-start mb-2">
                                  ...
                                  <div className="hidden group-hover:flex items-center gap-1">
                                      ...
                                      onClick={async () => {
                                        try {
                                          const auth = JSON.parse(
                                            localStorage.getItem('campuspool-auth') || '{}',
                                          );
                                          const res = await fetch(
                                            `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/drives/${driveId}/export/room/${rm._id}`,
                                            {
                                              headers: {
                                    ...
                                    <button className="p-1 text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 rounded ml-1">
                                      <Edit2 size={13} />
                                    </button>
                                    <button
                                      onClick={() => deleteRoomById(rm._id)}
                                      className="p-1 text-slate-400 hover:text-red-500 bg-slate-50 hover:bg-red-50 rounded ml-1"
                                    >
                                      <Trash2 size={13} />
                                    </button>
                                  </div>
                                ...
                                <div className="mt-2">
                                  <div className="flex justify-between text-xs text-slate-500 mb-1">
                                    <span>Capacity</span>
                                    <span>
                                      {rm.assignedStudents?.length || 0} / {rm.capacity} seats
                                    </span>
                                  </div>
                                  <div className="h-1.5 bg-slate-100 rounded-full">
                                    <div
                                      ...
                                      style={{
                                        width: `${Math.min(100, ((rm.assignedStudents?.length || 0) / rm.capacity) * 100)}%`,
                                      }}
                                    />
                                  </div>
                                </div>
                                ...
                                <div className="mt-2 flex flex-wrap gap-1">
                                  {rm.panelists?.map((p: any, i: number) => (
                                    <span key={i} className="inline-flex items-center gap-1">
                                      <span className="bg-indigo-50 text-indigo-600 text-xs px-2 py-0.5 rounded-full border border-indigo-200">
                                        {p.name}
                                      </span>
            ...
            {drive.status === 'event_day' && (
              <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-6">
                <h3 className="text-indigo-700 font-semibold text-lg flex items-center gap-2 mb-4">
                  <span className="w-2 h-2 rounded-full bg-indigo-600 animate-pulse" /> Live Event
                  Dashboard
                </h3>
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <div className="bg-white rounded-xl p-4 text-center border border-slate-200">
                    ...
                    <p className="text-3xl font-black text-amber-600">
                      {liveStats.roomsOpen || rooms.length}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        toast.error('Failed to start QR');
                      }
                    }}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    className="w-full border border-slate-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none resize-none"
                    rows={3}
                    placeholder="E.g., Requested by college placement cell"
                    value={extendReason}
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-5 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors shadow-sm disabled:opacity-50"
              >
                Extend Deadline
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-indigo-50/50">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-600 bg-white hover:bg-slate-100 p-1.5 rounded-lg transition-colors border shadow-sm"
              >
                <X size={16} />
              </button>
            </div>
            <div className="p-6">
              <p className="text-sm text-slate-600 mb-4 font-medium">
                Please set a new closing deadline to reopen this form for applications.
              </p>
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase mb-1">
                  New Close Date *
            ...
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="p-6">
                <p className="text-sm text-slate-600 font-medium">
                  Are you sure you want to close this form? Students will not be able to submit
                  after this.
                </p>
              </div>
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button
                  onClick={closeFormNow}
                  className="px-5 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors shadow-sm"
                >
                  Close Form
                </button>
              </div>
                      ...
                      <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white border border-slate-200 p-4 rounded-xl shadow-sm">
                        <div className="flex items-center justify-between space-x-2 mb-1">
                          <div className="font-bold text-slate-900 text-sm">
                            Extended By {ext.extendedBy}
                          </div>
                          <time className="text-xs font-medium text-slate-500">
                            {new Date(ext.extendedAt).toLocaleDateString()}
                          </time>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-xs font-bold mt-2 pt-2 border-t">
                          <span className="text-slate-400 line-through">
                            {ext.previousCloseDate
                              ? new Date(ext.previousCloseDate).toLocaleString()
                              : 'N/A'}
                          </span>
                          <span className="text-indigo-400">→</span>
                          <span className="text-indigo-600">
                            {new Date(ext.newCloseDate).toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              {lastWalkIn ? (
                <div className="text-center">
                  ...
                  <p className="text-slate-500 text-sm mb-4">
                    {lastWalkIn.name} has been fast-tracked in.
                  </p>
                  <div className="bg-emerald-50 border-2 border-emerald-200 rounded-2xl p-4 mb-5">
                    <p className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1">
                      Drive Student ID
                    </p>
                    <p className="text-3xl font-black text-emerald-700 tracking-widest font-mono">
                      {lastWalkIn.driveStudentId}
                    </p>
                    <p className="text-xs text-emerald-500 mt-1">
                      Share this ID with the student for check-in
                    </p>
                  </div>
                  <div className="flex gap-3">
                    <button
                ...
                <form onSubmit={handleWalkIn} className="space-y-4">
                  <p className="text-sm text-slate-500 font-medium -mt-1 mb-1">
                    Instant registration for unregistered students. They'll be marked as{' '}
                    <span className="font-bold text-emerald-600">Attended</span> immediately.
                  </p>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setWalkInForm((f) => ({ ...f, usn: e.target.value.toUpperCase() }))
                        }
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-2 items-start">
                    ...
                    <p className="text-xs text-amber-700 font-medium">
                      The drive must be in <strong>Event Day</strong> mode for walk-ins to work.
                    </p>
                  </div>
                  <button
                    type="submit"
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-black text-lg text-slate-800 flex items-center gap-2">
                <Copy size={18} className="text-indigo-600" /> Import Form Template
              </h3>
              <button
                onClick={() => setShowImportModal(false)}
                className="text-slate-400 hover:text-slate-600 bg-white hover:bg-slate-200 p-1.5 rounded-lg border border-slate-200 transition-colors"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-6">
              <p className="text-sm text-slate-600 mb-4 font-medium">
                Select a previous drive to clone its form structure.{' '}
                <strong className="text-amber-600">
                  Warning: This will overwrite your current unsaved fields.
                </strong>
              </p>
                  ...
                  >
                    ...
                    {allDrives.map((d) => (
                      <option key={d._id} value={d._id}>
                        {d.companyName} ({new Date(d.createdAt).toLocaleDateString()})
                      </option>
                    ))}
                  </select>
                </div>
      ...
      {activeTab === 'Audit Log' && <DriveAuditLog driveId={driveId || ''} />}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto px-1 pb-4">
              {formFields
                .filter((f) => f.type !== 'file_pdf' && f.type !== 'file_image')
                .map((f) => (
                  <div key={f.id}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setManualCandidateData({ ...manualCandidateData, [f.id]: e.target.value })
                      }
                    />
                  </div>
                ))}
      ...
      {activeTab === 'God View' &&
        (() => {
          ...
          const totalAssigned = rooms.reduce(
            (acc: number, r: any) => acc + (r.assignedStudents?.length || 0),
            0,
          );
          const totalCap = rooms.reduce((acc: number, r: any) => acc + (r.capacity || 0), 0);
          const overloadedRooms = rooms.filter(
            (r: any) => r.assignedStudents?.length >= r.capacity && r.capacity > 0,
          );
            ...
            <div className="p-6 h-full overflow-y-auto w-full max-w-7xl mx-auto pb-32 space-y-8">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3">
                    🗺️ God View
                    <span className="text-sm font-bold text-slate-400 bg-slate-100 px-3 py-1 rounded-full border border-slate-200 shadow-inner">
                      Live Room Heatmap
                    </span>
                  </h2>
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Real-time capacity tracking & operational control for{' '}
                    {drive?.companyName || 'the event'}.
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div className="bg-slate-900 px-4 py-2 rounded-xl flex items-center gap-3 shadow-xl">
                    <div
                      className="relative w-32 h-8 bg-slate-800 rounded-full tracking-wider border border-slate-700 overflow-hidden"
                      onMouseLeave={() => {
                        if (!isDrivePaused) setPanicProgress(0);
                      }}
                    >
                      <div
                        className="absolute inset-0 bg-red-600 transition-all duration-75"
                        style={{ width: `${isDrivePaused ? 100 : panicProgress}%` }}
                      ></div>
                      ...
                      {!isDrivePaused && (
                        <div
                          className="absolute inset-y-0 left-0 w-8 h-8 cursor-pointer rounded-full bg-slate-300 hover:bg-white shadow z-20 flex items-center justify-center"
                          onMouseDown={() => {
                            let p = 0;
                            const int = setInterval(() => {
                              p += 5;
                      ...
                      {isDrivePaused && (
                        <div
                          className="absolute inset-y-0 right-0 w-8 h-8 cursor-pointer rounded-full bg-white shadow flex items-center justify-center z-20 animate-pulse"
                          onClick={() => {
                            setIsDrivePaused(false);
                            setPanicProgress(0);
                          }}
                        >
                  ...
                  <button
                    onClick={handlePurgeStaleQueue}
                    disabled={isPurgingQueue}
                    className="bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 px-4 py-2 rounded-xl flex items-center gap-2 shadow-sm font-bold text-sm transition-all focus:ring-2 focus:ring-red-400"
                  >
                    <Trash2 size={16} />
                  ...
                  {/* Projector Display */}
                  <a
                    href={`/event/${driveId}/projector`}
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{ textDecoration: 'none' }}
                    className="bg-violet-50 hover:bg-violet-100 text-violet-700 border border-violet-200 px-4 py-2 rounded-xl flex items-center gap-2 shadow-sm font-bold text-sm transition-all"
                  >
                    <Monitor size={16} />
                    Projector Display
                  </a>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col justify-center">
                  <span className="text-slate-500 uppercase font-black text-[10px] tracking-wider mb-1">
                    Total Rooms Online
                  </span>
                  <div className="flex items-end gap-2">
                    ...
                    <span className="text-sm font-bold text-slate-400 mb-1">active</span>
                  </div>
                </div>
                <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col justify-center">
                  <span className="text-slate-500 uppercase font-black text-[10px] tracking-wider mb-1">
                  ...
                  <div className="w-full bg-slate-100 h-1.5 rounded-full mt-2 overflow-hidden">
                    <div
                      ...
                      style={{ width: `${totalCap ? (totalAssigned / totalCap) * 100 : 0}%` }}
                    ></div>
                  </div>
                </div>
                <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col justify-center">
                  ...
                  <div className="flex items-end gap-2">
                    <span
                      className={`text-3xl font-black leading-none ${overloadedRooms.length > 0 ? 'text-red-600' : 'text-emerald-500'}`}
                    >
                      {overloadedRooms.length}
                    </span>
                    {overloadedRooms.length > 0 && (
                      <span className="text-sm font-bold text-red-400 mb-1 animate-pulse">
                        Needs attention
                      </span>
                    )}
                  </div>
                ...
                <div
                  className={`border-2 rounded-2xl p-4 shadow-sm flex flex-col justify-center transition-colors ${isDrivePaused ? 'bg-red-50 border-red-500' : 'bg-emerald-50 border-emerald-400'}`}
                >
                  <span
                    className={`uppercase font-black text-[10px] tracking-wider mb-1 ${isDrivePaused ? 'text-red-600' : 'text-emerald-700'}`}
                  >
                    System Status
                  </span>
                  <div className="flex items-end gap-2">
                    <span
                      className={`text-2xl font-black leading-none ${isDrivePaused ? 'text-red-700' : 'text-emerald-800'}`}
                    >
                      {isDrivePaused ? 'HALTED' : 'NOMINAL'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-white border-2 border-orange-200 rounded-2xl p-5 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${miaStudents.length > 0 ? 'bg-orange-500 animate-pulse' : 'bg-slate-300'}`}
                      />
                  ...
                  {miaStudents.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {miaStudents.map((s: any) => (
                        <div
                          key={s._id}
                          className="flex items-center justify-between bg-orange-50 border border-orange-200 rounded-xl px-3 py-2"
                        >
                          <div>
                            <p className="font-bold text-slate-800 text-sm">{s.name}</p>
                            <p className="text-xs text-slate-500">
                              {s.usn} · {s.branch} · Round {s.currentRound || '?'}
                            </p>
                          </div>
                ...
                <div className="bg-white border-2 border-amber-200 rounded-2xl p-5 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${latecomers.length > 0 ? 'bg-amber-500 animate-pulse' : 'bg-slate-300'}`}
                      />
                      <h3 className="font-black text-sm text-slate-700 uppercase tracking-wide">
                        ⏰ Latecomers on Hold
                        {latecomers.length > 0 && (
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {latecomers.map((s: any) => (
                        ...
                        >
                          <div>
                            <p className="font-bold text-slate-800 text-sm">
                              {s.data?.fullName || s.data?.name || s.name || 'Unknown'}
                            </p>
                            <p className="text-xs text-slate-500">
                              {s.data?.usn || s.usn || '—'} · {s.driveStudentId || '—'}
                            </p>
                          </div>
                          <button
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {roundGroups[round].map((room) => {
                        ...
                        const isDormant =
                          ass > 0 &&
                          room.updatedAt &&
                          new Date().getTime() - new Date(room.updatedAt).getTime() >
                            20 * 60 * 1000;
                        if (isDormant) {
                          st = {
                            bg: 'bg-purple-50',
                        ...
                        return (
                          ...
                          >
                            {isDormant && (
                              <div
                                className="absolute -top-3 -left-3 bg-purple-600 text-white p-1.5 rounded-full shadow-lg border-2 border-white animate-bounce z-10"
                                title="No activity in 20+ mins"
                              >
                            ...
                            <div className="flex justify-between items-start mb-4">
                              <div>
                                <h4 className={`font-black text-lg ${st.text} leading-none mb-1`}>
                                  {room.name}
                                </h4>
                              ...
                              <div className="flex flex-col items-end gap-2">
                                <div
                                  className={`px-2 py-1 rounded border text-[10px] font-black uppercase tracking-wider bg-white/50 ${st.text} border-current opacity-70`}
                                >
                                  {st.label}
                                </div>
                                <div
                                  className={`flex items-center bg-white/60 border rounded shadow-sm opacity-0 group-hover:opacity-100 transition-opacity ${st.border}`}
                                >
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleAdjustCapacity(room._id, -5);
                                    }}
                                    className={`p-1 hover:bg-slate-100 text-slate-600 hover:text-slate-900 border-r ${st.border}`}
                                  >
                                    <Minus size={12} strokeWidth={3} />
                                  </button>
                                  <div className={`text-[10px] font-black px-1 text-slate-500`}>
                                    CAP
                                  </div>
                                  <button
                                    onClick={(e) => {
                            ...
                            <div className="flex items-end justify-between">
                              <div className="w-full">
                                <div className="flex items-baseline gap-1 mb-1">
                                  ...
                                  <span className={`font-bold text-xs opacity-50 ${st.text}`}>
                                    / {cap || '?'}
                                  </span>
                                </div>
                                <div className="w-full bg-black/5 h-1.5 rounded-full overflow-hidden">
                                  <div
                                    className={`h-full rounded-full transition-all duration-500 ${st.morph || st.pulse ? 'bg-red-500' : 'bg-current opacity-50'}`}
                                    style={{
                                      width: `${cap ? Math.min((ass / cap) * 100, 100) : 0}%`,
                                    }}
                                  ></div>
                                </div>
                              </div>
              ...
              <div className="fixed bottom-0 left-0 right-0 md:left-64 bg-slate-900 border-t border-slate-800 py-3 px-6 z-40 shadow-2xl flex items-center justify-between">
                <div className="flex items-center gap-4 overflow-hidden mask-fade-right">
                  <span className="text-[10px] font-black text-slate-400 bg-slate-800 px-2 py-1 rounded uppercase tracking-widest shrink-0">
                    Live Log
                  </span>
                  <div
                    className="flex items-center gap-6 whitespace-nowrap overflow-x-auto no-scrollbar"
                  ...
                  >
                    {liveEvents.map((le) => (
                      ...
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${le.type === 'info' ? 'bg-indigo-400' : le.type === 'warning' ? 'bg-orange-400' : 'bg-red-500 animate-pulse'}`}
                        ></div>
                        {le.text}
                      </div>
                    ))}
              ...
              <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row transition-all max-h-[80vh]">
                <div className="bg-slate-50 border-r border-slate-200 p-8 w-full md:w-1/3 flex flex-col">
                  <button
                    ...
                    className="self-start text-slate-400 hover:text-slate-600 mb-6 bg-white border rounded-full p-2 hover:shadow-sm transition-all"
                  >
                    <ArrowLeft size={16} />
                  </button>
                    ...
                    {room.panelists ? (
                      <div className="space-y-2 mt-2">
                        {room.panelists.split(',').map((p: string, i: number) => (
                          <div
                            key={i}
                            className="flex items-center gap-2 text-sm font-bold text-slate-700 bg-slate-50 px-3 py-2 rounded-lg border border-slate-100"
                          >
                ...
                <div className="p-8 w-full md:w-2/3 bg-white overflow-y-auto">
                  <h3 className="font-black text-slate-800 mb-4 flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-emerald-500"></div> Candidate Ledger
                  </h3>
                  {!room.assignedStudents || room.assignedStudents.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {room.assignedStudents.map((st: any, i: number) => (
                        <div
                          key={i}
                          className="flex items-center justify-between p-3 rounded-xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/50 transition-colors"
                        >
      ...
      {showAIMatchModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          />
          <div className="bg-white w-full max-w-4xl max-h-[85vh] rounded-3xl shadow-2xl relative flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {/* Header */}
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-violet-600 to-indigo-600">
              <div className="flex items-center gap-3 text-white">
                <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center backdrop-blur-md">
                  <Sparkles size={20} className="text-white" />
                </div>
                <div>
                  <h3 className="text-lg font-bold leading-none mb-1">AI Candidate Match</h3>
                  <p className="text-sm text-indigo-100 font-medium">
                    Powered by Qdrant & Nomic Embeddings
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50 custom-scrollbar">
              {isMatching ? (
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <p className="mt-6 font-bold text-slate-800">Embedding Job Description...</p>
                  <p className="text-sm font-medium text-slate-500 mt-1">
                    Running cosine similarity search across candidate vectors
                  </p>
                </div>
              ) : !aiMatchData ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                      <div className="text-sm font-bold text-slate-400 mb-1">Total Found</div>
                      <div className="text-2xl font-black text-slate-800">
                        {aiMatchData.totalFound}
                      </div>
                    </div>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                      <div className="text-sm font-bold text-slate-400 mb-1">Already Applied</div>
                      <div className="text-2xl font-black text-indigo-600">
                        {aiMatchData.alreadyApplied}
                      </div>
                    </div>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                      <div className="text-sm font-bold text-slate-400 mb-1">New Matches</div>
                      <div className="text-2xl font-black text-emerald-600">
                        {aiMatchData.matchedCandidates?.length || 0}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                      <h4 className="font-bold text-slate-800">Top Matches</h4>
                      <span className="text-xs font-bold text-slate-400 px-2.5 py-1 bg-white border border-slate-200 rounded-lg shadow-sm">
                    ...
                    <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                      {aiMatchData.matchedCandidates?.length === 0 ? (
                        <div className="p-8 text-center text-slate-500 font-medium">
                          No new candidates found matching the criteria.
                        </div>
                      ) : (
                        <div className="divide-y divide-slate-50">
                          {aiMatchData.matchedCandidates?.map((c: any, i: number) => {
                            ...
                            return (
                              ...
                              >
                                <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center font-bold text-slate-400 shrink-0">
                                  #{i + 1}
                                </div>
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-3 text-sm font-medium text-slate-500 mt-1">
                                    <span className="truncate">{c.branch || 'N/A'}</span>
                                    <span className="w-1 h-1 rounded-full bg-slate-300 shrink-0" />
                                    <span>{c.cgpa} CGPA</span>
                                  </div>
                                </div>
                                <div className="shrink-0 flex items-center gap-3">
                                  <div className="text-right">
                                    <div
                                      className={`font-black text-lg ${matchPercent > 80 ? 'text-emerald-600' : matchPercent > 60 ? 'text-amber-500' : 'text-slate-400'}`}
                                    >
                                      {matchPercent}%
                                    </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-white shrink-0">
              ...
              {aiMatchData?.matchedCandidates?.length > 0 && !isMatching && (
                <button className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-md shadow-indigo-200 transition-all hover:-translate-y-0.5">
