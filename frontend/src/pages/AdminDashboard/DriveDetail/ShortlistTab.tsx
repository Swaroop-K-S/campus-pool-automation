...
export default function ShortlistTab() {
  ...
  const handleUpload = async () => {
    ...
    try {
      // Send raw XLSX to FastAPI backend
      const response = await fetch(`/api/drives/${id}/shortlist/upload`, {
        method: 'POST',
        body: formData,
      });
        ...
        <div className="border-2 border-dashed border-border rounded-xl p-10 flex flex-col items-center justify-center bg-background transition-colors hover:bg-secondary/20">
          {uploadStatus === 'success' ? (
            <div className="text-center">
              ...
              <p className="text-muted-foreground mt-1">The student data has been parsed and saved.</p>
              <button 
