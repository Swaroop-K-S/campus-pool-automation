...
export default function StudentsPanel() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!studentsRes.ok) throw new Error('Failed to load global student database');
      if (!drivesRes.ok) throw new Error('Failed to load drive directories');
      ...
      setError(err.message || 'Failed to fetch candidate directory data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredStudents = students.filter((s) => {
    ...
    const matchesDrive = driveFilter === 'all' || s.drive_id === driveFilter;
  ...
  const handleExportExcel = () => {
    ...
    if (driveFilter && driveFilter !== 'all') params.append('drive_id', driveFilter);

    window.open(`/api/v1/students/export?${params.toString()}`, '_blank');
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
        ...
        <span className="font-medium text-sm">Loading students directory...</span>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Global Students Directory</h2>
          <p className="text-sm text-muted-foreground mt-0.5">
            Total Candidates: {students.length} | Displaying: {filteredStudents.length}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div className="sm:col-span-2 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-card border border-border rounded-lg text-sm focus:outline-none focus:border-primary transition-colors text-foreground"
          />
        </div>
        ...
        <div>
          <select
            ...
            className="w-full px-3 py-2 bg-card border border-border rounded-lg text-sm focus:outline-none focus:border-primary transition-colors text-foreground"
          >
                  ...
                  <React.Fragment key={s.id}>
                    ...
                    {isExpanded && (
                      <tr className="bg-secondary/20">
                        <td colSpan={6} className="px-8 py-4 border-t border-b border-border shadow-inner">
